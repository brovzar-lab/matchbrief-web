...
export const scheduleLegacyDelivery = functions.pubsub
  ...
  .onRun(async (_context) => {
    ...
    for (const doc of snapshot.docs) {
      ...
      const emailBody =
        data.type === 'text'
          ? buildTextEmail(data.content as string, data.title as string)
          : buildMediaEmail(data.type as string, data.storageRef as string, data.title as string);

      emailPromises.push(
        sgMail.send({
          to: toList,
          from: { name: 'LegacyLetter', email: 'noreply@legacyletter.app' },
          subject: `A legacy message for you: "${data.title}"`,
          html: emailBody,
        })
      );
...
function buildMediaEmail(type: string, storageRef: string, title: string): string {
  ...
  return `
    <div style="font-family: Georgia, serif; max-width: 640px; margin: 0 auto; padding: 40px 24px; color: #1a1a1a;">
      ...
      <div style="background: #f5f5f5; border-radius: 12px; padding: 24px; text-align: center;">
        ...
        <p style="font-size: 18px; font-weight: 600; color: #333;">${label} Attached</p>
        <p style="font-size: 14px; color: #888;">Reference: ${storageRef}</p>
        <p style="font-size: 13px; color: #aaa; margin-top: 16px;">
          TODO: Replace with a signed download URL valid for 30 days.
        </p>
      </div>
      <hr style="margin: 40px 0; border: none; border-top: 1px solid #eee;">
      <p style="font-size: 12px; color: #aaa; text-align: center;">
